...
use adapter_core::{
    AdapterCore, Caller, CallerId, CoreCommand, CoreEvent, CoreEventKind,
...
pub enum A2aStreamEvent {
    TaskStatusUpdate { task_id: String, status: A2aTaskStatus, final_update: bool },
    TaskArtifactUpdate { task_id: String, artifact: A2aArtifact },
}
...
pub trait A2aCoreService: Send + Sync {
    ...
    async fn subscribe(&self, task_id: TaskId, after_seq: u64)
...
impl<C: A2aCoreService> A2aMapper<C> {
    pub fn new(core: Arc<C>, card: AgentCard) -> Self { Self { core, card } }
    pub fn agent_card(&self) -> &AgentCard { &self.card }
    ...
    pub async fn send_task(
        ...
        match result {
            ...
            _ => Err(A2aMapperError::InvalidRequest("unexpected core result for invoke".into())),
        }
    }

    pub async fn get_task(&self, caller: Caller, task_id: &str) -> Result<A2aTask, A2aMapperError> {
        let task_id = parse_uuid(task_id, "task_id")?;
        let result = self.core.dispatch(caller, CoreCommand::GetStatus { task_id }).await?;
        match result {
            ...
            _ => Err(A2aMapperError::InvalidRequest("unexpected core result for status".into())),
        }
    }

    pub async fn cancel_task(&self, caller: Caller, task_id: &str) -> Result<A2aTask, A2aMapperError> {
        let task_id = parse_uuid(task_id, "task_id")?;
        let result = self.core.dispatch(caller, CoreCommand::Cancel { task_id, reason: None }).await?;
        match result {
            ...
            _ => Err(A2aMapperError::InvalidRequest("unexpected core result for cancel".into())),
        }
    }
    ...
    pub async fn subscribe_task(
        ...
        let _ = self.core.dispatch(caller, CoreCommand::GetStatus { task_id }).await?;
...
fn to_core_invoke(request: A2aTaskRequest) -> Result<InvokeRequest, A2aMapperError> {
    if request.idempotency_key.trim().is_empty() {
        return Err(A2aMapperError::InvalidRequest("idempotency_key is required".into()));
    }
    let input = request.message.parts.into_iter().map(to_core_part).collect::<Result<Vec<_>, _>>()?;
    let session_id = request.context_id.as_deref().map(|value| parse_uuid(value, "context_id")).transpose()?;
    Ok(InvokeRequest {
...
fn to_core_part(part: A2aPart) -> Result<Part, A2aMapperError> {
    match part.kind.as_str() {
        "text" => Ok(Part::Text { text: part.text.unwrap_or_default() }),
        "data" | "json" => Ok(Part::Json { value: part.data.unwrap_or(serde_json::Value::Null) }),
        "file" | "file_ref" => Ok(Part::FileRef {
            uri: part.uri.ok_or_else(|| A2aMapperError::InvalidRequest("file part requires uri".into()))?,
            mime_type: part.mime_type,
        }),
        kind => Err(A2aMapperError::InvalidRequest(format!("unsupported A2A part kind: {kind}"))),
    }
}
    ...
    match event.kind {
        CoreEventKind::Artifact { artifact } => A2aStreamEvent::TaskArtifactUpdate {
            ...
            artifact: A2aArtifact {
                ...
                parts: vec![A2aPart { kind: "file_ref".into(), text: None, data: None, uri: artifact.uri, mime_type: None }],
            },
        },
        kind => {
            let (state, message, final_update) = match kind {
                CoreEventKind::Accepted { .. } => ("submitted", None, false),
                CoreEventKind::Progress { message, .. } => ("working", Some(text_message(message)), false),
                CoreEventKind::InputRequired { request } => ("input-required", Some(text_message(request.question)), false),
                CoreEventKind::Completed { output } => ("completed", Some(A2aMessage { role: "agent".into(), parts: output.into_iter().map(from_core_part).collect() }), true),
                CoreEventKind::Failed { error } => ("failed", Some(text_message(error.message)), true),
                CoreEventKind::CancelRequested { .. } => ("working", Some(text_message("cancellation requested".into())), false),
                CoreEventKind::Cancelled => ("canceled", None, true),
                CoreEventKind::Artifact { .. } => unreachable!(),
            };
            A2aStreamEvent::TaskStatusUpdate { task_id, status: A2aTaskStatus { state: state.into(), message }, final_update }
        }
    }
}
...
fn status_from_snapshot(snapshot: &adapter_core::TaskSnapshot) -> A2aTaskStatus {
    ...
    A2aTaskStatus { state: state.into(), message: None }
}

fn from_core_part(part: Part) -> A2aPart {
    match part {
        Part::Text { text } => A2aPart { kind: "text".into(), text: Some(text), data: None, uri: None, mime_type: None },
        Part::Json { value } => A2aPart { kind: "data".into(), text: None, data: Some(value), uri: None, mime_type: None },
        Part::FileRef { uri, mime_type } => A2aPart { kind: "file_ref".into(), text: None, data: None, uri: Some(uri), mime_type },
    }
}

fn text_message(text: String) -> A2aMessage {
    A2aMessage { role: "agent".into(), parts: vec![A2aPart { kind: "text".into(), text: Some(text), data: None, uri: None, mime_type: None }] }
}

fn parse_uuid(value: &str, field: &str) -> Result<Uuid, A2aMapperError> {
    value.parse().map_err(|_| A2aMapperError::InvalidRequest(format!("{field} must be UUID")))
}
