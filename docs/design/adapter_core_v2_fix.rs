...
use std::sync::Arc;
use adapter_model::{AgentId, AgentLimits, Caller, CoreCommand, CoreEvent, CoreEventKind, CreateTaskResult, DispatchResult, DriverCapabilities, EventSeq, InvokeRequest, NewTask, Part, PublicError, TaskId, TaskSnapshot, TaskState, TaskTransition};
...
impl AgentRegistry {
    ...
    pub fn get(&self, id: &AgentId) -> Option<Arc<RegisteredAgent>> { self.agents.get(id).map(|v| v.clone()) }
    pub fn resolve(&self, request: &InvokeRequest) -> Result<Arc<RegisteredAgent>, CoreError> {
        if let Some(id) = &request.agent_id { return self.get(id).ok_or_else(|| CoreError::AgentNotFound(id.0.clone())); }
...
#[async_trait]
pub trait PolicyEngine: Send + Sync { async fn authorize(&self, caller: &Caller, command: &CoreCommand) -> Result<(), CoreError>; }
pub struct AllowAllPolicy;
#[async_trait] impl PolicyEngine for AllowAllPolicy { async fn authorize(&self, _: &Caller, _: &CoreCommand) -> Result<(), CoreError> { Ok(()) } }

struct ActiveTask { tx: broadcast::Sender<CoreEvent>, cancellation: CancellationToken }
pub struct TaskSubscription { pub history: Vec<CoreEvent>, pub receiver: broadcast::Receiver<CoreEvent> }
...
#[derive(Clone)]
pub struct AdapterCore { inner: Arc<CoreInner> }

impl AdapterCore {
    pub fn new(store: Arc<dyn TaskStore>, registry: Arc<AgentRegistry>, policy: Arc<dyn PolicyEngine>, max_concurrent: usize) -> Self {
        Self { inner: Arc::new(CoreInner { store, registry, policy, global_permits: Arc::new(Semaphore::new(max_concurrent)), active: DashMap::new() }) }
    }
    ...
    pub async fn subscribe(&self, task_id: TaskId, after_seq: EventSeq) -> Result<TaskSubscription, CoreError> {
        let history = self.inner.store.events_after(task_id, after_seq, 500).await?;
        let receiver = self.inner.active.get(&task_id).map(|entry| entry.tx.subscribe()).unwrap_or_else(closed_receiver);
    ...
    async fn invoke(&self, caller: Caller, request: InvokeRequest) -> Result<DispatchResult, CoreError> {
        ...
        let initial = self.inner.store.create_or_get_idempotent(NewTask {
        ...
        let snapshot = match initial { CreateTaskResult::Existing(s) => return Ok(DispatchResult::Existing(s)), CreateTaskResult::Created(s) => s };
        let (tx, _) = broadcast::channel(256);
        self.inner.active.insert(task_id, ActiveTask { tx, cancellation: CancellationToken::new() });
        ...
        let global = match self.inner.global_permits.clone().try_acquire_owned() {
            Ok(permit) => permit,
            Err(_) => { self.fail_active(task_id, CoreError::ResourceExhausted("global task limit reached".into())).await?; return Err(CoreError::ResourceExhausted("global task limit reached".into())); }
        };
        let per_agent = match agent.permits.clone().try_acquire_owned() {
            Ok(permit) => permit,
            Err(_) => { self.fail_active(task_id, CoreError::ResourceExhausted(format!("agent {} busy", agent.id.0))).await?; return Err(CoreError::ResourceExhausted(format!("agent {} busy", agent.id.0))); }
        };
        let core = self.clone();
        tokio::spawn(async move {
            let _global = global;
            let _agent = per_agent;
            if let Err(error) = core.run_driver(agent, task_id, request).await { let _ = core.fail_active(task_id, error).await; }
    ...
    async fn cancel(&self, task_id: TaskId, reason: Option<String>) -> Result<DispatchResult, CoreError> {
        ...
        let agent = self.inner.registry.get(&snapshot.agent_id).ok_or_else(|| CoreError::AgentNotFound(snapshot.agent_id.0.clone()))?;
        ...
        if let Some(active) = self.inner.active.get(&task_id) { active.cancellation.cancel(); }
    ...
    async fn run_driver(&self, agent: Arc<RegisteredAgent>, task_id: TaskId, request: InvokeRequest) -> Result<(), CoreError> {
        ...
        if !self.snapshot(task_id).await?.state.terminal() { return Err(CoreError::Driver("driver stream closed before terminal event".into())); }
        Ok(())
    }
    ...
    async fn apply_driver_event(&self, task_id: TaskId, event: DriverEvent) -> Result<(), CoreError> {
        ...
        match event {
            ...
            DriverEvent::Progress { message, percent } => { let next = if snapshot.state == TaskState::Accepted { TaskState::Running } else { snapshot.state }; self.transition(snapshot, vec![TaskState::Accepted, TaskState::Running], next, CoreEventKind::Progress { message, percent }).await?; Ok(()) }
    ...
    async fn transition(&self, snapshot: TaskSnapshot, allowed: Vec<TaskState>, next: TaskState, kind: CoreEventKind) -> Result<TaskSnapshot, CoreError> {
        let applied = self.inner.store.append_event_and_transition(TaskTransition { task_id: snapshot.task_id, expected_revision: snapshot.revision, allowed_states: allowed, next_state: next, event_kind: kind }).await?;
        if let Some(active) = self.inner.active.get(&snapshot.task_id) { let _ = active.tx.send(applied.event); }
        Ok(applied.snapshot)
    }

    async fn snapshot(&self, task_id: TaskId) -> Result<TaskSnapshot, CoreError> {
        self.inner.store.get_snapshot(task_id).await?.ok_or_else(|| CoreError::Store(StoreError::NotFound(task_id)))
    }
}

fn closed_receiver() -> broadcast::Receiver<CoreEvent> { let (tx, rx) = broadcast::channel(1); drop(tx); rx }
