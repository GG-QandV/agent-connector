...
//! platform service registration -> InstallSummary.
//!
//! Контракт с adapterd, подтверждённый в config-2.rs/main.rs:
//!   StorageConfig::Sqlite { path }                      — просто файл
//!   StorageConfig::Postgres { dsn_env, schema, max_connections }
//!     -> adapterd делает env::var(dsn_env) при старте (main.rs build_store())
//!     -> installer НИКОГДА не пишет сам DSN в adapter.yaml, только имя
//!        переменной; секрет уходит в .env под этим именем.
...
pub struct ResolvedStorage {
    /// Точный YAML-фрагмент для секции `storage:` в adapter.yaml.
    pub yaml_fragment: serde_yaml::Value,
    /// (имя переменной, значение) для записи в .env. Пусто для Sqlite/Memory.
    pub env_vars: Vec<(String, String)>,
    /// Для managed-docker — что показать в финальной сводке.
...
/// при наличии TTY. В CI/скриптах без TTY и без --storage — явная ошибка,
/// не тихий дефолт на что-то потенциально нежелательное (например, Sqlite,
/// когда пользователь на самом деле хотел Postgres).
pub fn resolve_storage_choice(flag: Option<StorageChoice>) -> Result<StorageChoice, InstallError> {
    ...
    println!("  1) SQLite            — single file, no external dependencies, good for single-node/dev");
    ...
    match input.trim() {
        ...
        other => Err(InstallError::Storage(format!("invalid choice '{other}', expected 1-4"))),
    }
}

/// Шаг 2: по выбору строит ResolvedStorage. Для Postgres-вариантов —
/// РЕАЛЬНО проверяет соединение (`SELECT 1`) до того, как считать
/// установку успешной — не просто принимает DSN на веру.
pub async fn resolve_storage(
    ...
    match choice {
        ...
        StorageChoice::ManagedDockerPostgres => {
            let plan: ManagedPostgresPlan = managed_docker::plan(confirm_docker, docker_network_only, DSN_ENV_VAR_NAME)
...
/// Реальная проверка `SELECT 1` через tokio-postgres, не просто "DSN
/// выглядит как валидный URL". Даёт installer'у шанс зафейлиться понятно
/// ДО регистрации systemd/sc.exe службы, а не после первого крэша adapterd
/// в проде из-за опечатки в пароле.
async fn validate_postgres_connection(dsn: &str) -> Result<(), InstallError> {
    ...
    // Драйверу нужен фоновый task для обслуживания соединения — стандартный
    // паттерн tokio_postgres, коннекшен сам закроется когда client уйдёт из scope.
...
/// Шаг 3: пишет adapter.yaml (без секретов) и .env (с секретами, chmod/icacls
/// применяется platform-слоем СРАЗУ после записи, до того как файл может
/// быть прочитан кем-то ещё).
pub fn write_config_files(
    ...
    // Права ограничиваются немедленно после записи — правило #5.
...
pub async fn run_install(
    ...
    if choice == StorageChoice::ManagedDockerPostgres && !confirm_docker {
        // Дублирует проверку внутри managed_docker::plan(), но фейлимся
        // здесь раньше — до platform.ensure_service_user()/build — чтобы
        // не тратить время на сборку, если сам storage-выбор невалиден.
    ...
    // Плейсхолдер: минимальный agents YAML. Реальная версия должна брать
    // это из --config шаблона (как в текущем install.sh через --config
    // path/to/adapter.yaml), здесь для целостности install-flow берём
    // единственный echo-агент как smoke-конфиг по умолчанию.
    let default_agents = serde_yaml::to_value(serde_json::json!([{
        "id": "echo",
        "skills": ["echo"],
        "driver": "stdio",
        "command": "echo",
    ...
    let binary_src = std::env::current_dir()?.join("target/release/adapterd");
    ...
    let (config_path, env_path) = write_config_files(&prefix, &resolved, &default_agents, platform.as_ref())?;
...
fn render_summary(
    ...
    if let Some(docker) = &resolved.docker_summary {
        if let Some(port) = docker.host_port {
            out.push_str(&format!("  postgres:  127.0.0.1:{port} (container: {})\n", docker.container_name));
        } else {
            out.push_str(&format!("  postgres:  internal Docker network only (container: {})\n", docker.container_name));
        }
        out.push_str(&format!("  pg volume: {} (NOT removed on uninstall unless --purge-data)\n", docker.volume_name));
        out.push_str(&format!(
    ...
    out.push_str(&format!(
        "  started:   {}\n",
        if started { "yes" } else { "no — run `adapterctl start` or the platform service command" }
    ));
    out.push_str(&format!(
...
mod tests {
    ...
    #[test]
    fn resolve_storage_choice_errors_without_tty_or_flag() {
        // Примечание: этот тест валиден только когда реально запущен без
        // TTY (типичный случай CI runner) — если запускать локально в
        // интерактивном терминале, atty::is вернёт true и тест нужно
        // будет обернуть в мок atty для детерминизма. Отмечено как известное
        // ограничение теста, не架构 install_flow.
        if atty::is(atty::Stream::Stdin) {
