...
//!     это разница принципиальна: bootstrap корректно работает с System
//!     Integrity Protection и новыми ограничениями на background items
//!     (macOS 13+ требует явного approval через System Settings для
//!     некоторых launchd jobs, но LaunchDaemons от root обычно не требуют
//!     интерактивного approval, в отличие от LaunchAgents).
//!   - System account UID диапазон 200-400 (стандартный macOS диапазон
//!     для system services, не 1-199 который зарезервирован под сам ОС,
//!     не 501+ который для обычных пользователей).
//!   - LaunchDaemon (не LaunchAgent) — daemon должен работать без
//!     залогиненного пользователя, аналогично systemd system service,
//!     не user session service.
...
/// не пересекается ни с самой ОС (1-199), ни с обычными пользователями (501+).
...
impl PlatformServiceManager for Launchd {
    fn ensure_service_user(&self, name: &str) -> Result<(), PlatformError> {
        ...
        // Порядок важен: dscl -create без атрибутов создаёт "голую" запись,
        // затем UniqueID/PrimaryGroupID/UserShell/NFSHomeDirectory
        // добавляются отдельными вызовами (dscl API так работает, не
        // принимает все атрибуты одной командой).
        run_checked("dscl", &[".", "-create", &user_path], "dscl create failed")?;
        run_checked("dscl", &[".", "-create", &user_path, "UniqueID", &uid.to_string()], "dscl set UniqueID failed")?;
        run_checked("dscl", &[".", "-create", &user_path, "PrimaryGroupID", "1"], "dscl set PrimaryGroupID failed")?; // gid 1 = daemon group
        run_checked("dscl", &[".", "-create", &user_path, "UserShell", "/usr/bin/false"], "dscl set UserShell failed")?;
        run_checked("dscl", &[".", "-create", &user_path, "NFSHomeDirectory", "/var/empty"], "dscl set NFSHomeDirectory failed")?;
        run_checked("dscl", &[".", "-create", &user_path, "RealName", "agent-connector adapterd service account"], "dscl set RealName failed")?;
        // IsHidden скрывает аккаунт из UI логина/Fast User Switching —
        // это service account, не должен предлагаться как вариант логина.
        run_checked("dscl", &[".", "-create", &user_path, "IsHidden", "1"], "dscl set IsHidden failed")?;
    ...
    fn register_service(&self, ctx: &ServiceContext) -> Result<(), PlatformError> {
        ...
        // plist с неправильными правами/владельцем (документированное
        // поведение, не просто best practice).
        run_checked("chown", &["root:wheel", PLIST_PATH], "chown on plist failed")?;
        std::fs::set_permissions(PLIST_PATH, std::fs::Permissions::from_mode_workaround(0o644))
        ...
        // не устаревший `launchctl load`. system domain = daemon работает
        // независимо от залогиненной сессии, как systemd system unit.
        run_checked(
    ...
    fn unregister_service(&self, _name: &str) -> Result<(), PlatformError> {
        ...
        // bootout может вернуть ошибку, если сервис уже не загружен —
        // не фейлим на этом, только на удалении файла (та же логика,
        // что в Linux/Windows unregister_service).
    ...
    fn start_service(&self, _name: &str) -> Result<(), PlatformError> {
        ...
        // гарантирует чистый старт даже если предыдущий процесс завис.
        run_checked(
    ...
    fn restrict_file_permissions(&self, path: &Path, owner: &str) -> Result<(), PlatformError> {
        ...
        run_checked("chown", &[&format!("{owner}:daemon"), path.to_str().unwrap_or("")], "chown failed")
    }
}

/// Генерирует .plist XML. ProgramArguments = [binary_path, config_path] —
/// зеркалит ExecStart из systemd unit (позиционный аргумент конфига).
/// KeepAlive с SuccessfulExit:false — restart только при ненулевом коде
/// выхода, аналог Restart=on-failure (не restart на чистый exit 0, в
/// отличие от KeepAlive:true unconditional, который перезапустил бы даже
/// после штатной остановки).
fn render_plist(ctx: &ServiceContext) -> Result<String, PlatformError> {
    let binary = ctx.binary_path.to_str()
        .ok_or_else(|| PlatformError::CommandFailed("non-UTF8 binary path".into()))?;
    let config = ctx.config_path.to_str()
        .ok_or_else(|| PlatformError::CommandFailed("non-UTF8 config path".into()))?;
    let working_dir = ctx.working_directory.to_str()
        .ok_or_else(|| PlatformError::CommandFailed("non-UTF8 working directory".into()))?;
    let data_dir = ctx.data_directory.to_str()
        .ok_or_else(|| PlatformError::CommandFailed("non-UTF8 data directory".into()))?;
    let env_file = ctx.env_file_path.to_str()
        .ok_or_else(|| PlatformError::CommandFailed("non-UTF8 env file path".into()))?;

    // EnvironmentVariables в plist — статический словарь, launchd не умеет
    // читать .env файл напрямую (в отличие от systemd EnvironmentFile=).
    // Решение: adapterd сам должен уметь читать .env файл при старте
    // (dotenv-подобная логика в main.rs), НЕ полагаться на launchd за это —
    // отличие от Linux, где EnvironmentFile= делает systemd. Здесь передаём
    // путь к .env как аргумент/переменную, чтобы adapterd прочитал его сам.
    Ok(format!(
...
<dict>
    ...
    <!-- Аналог ReadWritePaths+ProtectSystem=full из systemd unit: launchd
         ...
         profile мог бы, но это отдельный, более сложный механизм) — вместо
         этого полагаемся на Unix-права: {data_dir} принадлежит {user},
         остальное — root:daemon read-only (см. restrict_file_permissions). -->
</dict>
</plist>
"#,
...
/// перебор с dscl -search — приемлемо, потому что выполняется один раз
/// при install, не в hot path.
...
fn require_root() -> Result<(), PlatformError> {
    // На macOS geteuid() тоже стабильный libc symbol, тот же приём, что
    // на Linux — не требует отдельной crate-зависимости.
...
fn run_checked(cmd: &str, args: &[&str], err_context: &str) -> Result<(), PlatformError> {
    ...
    if !output.status.success() {
        ...
        return Err(PlatformError::CommandFailed(format!(
            "{err_context}: {cmd} exited with {} — {stderr}", output.status
        )));
    }
    Ok(())
}
...
mod tests {
    ...
    fn plist_contains_modern_keepalive_dict_not_bare_true() {
        ...
        // KeepAlive должен быть dict с SuccessfulExit:false, НЕ bare <true/> —
        // bare true перезапускает даже после штатного exit 0, что нежелательно
        // при плановой остановке через launchctl kickstart без -k.
    ...
    fn system_uid_range_excludes_os_reserved_and_regular_users() {
        ...
        assert!(!SYSTEM_UID_RANGE.contains(&100), "1-199 reserved for OS itself");
        assert!(!SYSTEM_UID_RANGE.contains(&501), "501+ reserved for regular user accounts");
