...
impl Default for StorageConfig {
    fn default() -> Self { Self::Sqlite { path: default_sqlite_path() } }
}
...
impl Default for RuntimeConfig {
    fn default() -> Self { Self { max_concurrent_tasks: default_global_tasks(), cleanup_interval_seconds: default_cleanup_seconds(), shutdown_grace_seconds: default_shutdown_seconds() } }
}
...
impl Default for RetentionConfig {
    fn default() -> Self { Self { task_ttl_days: default_task_days(), event_ttl_days: default_event_days(), idempotency_ttl_hours: default_key_hours(), cleanup_batch_size: default_cleanup_batch() } }
}
...
impl Default for AgentLimitsConfig {
    fn default() -> Self { Self { max_concurrent_tasks: default_agent_tasks(), max_queued_tasks: default_agent_queue(), max_input_bytes: default_input_bytes(), max_event_bytes: default_event_bytes(), default_timeout_seconds: default_task_timeout() } }
}
...
pub enum ConfigError {
    #[error("cannot read config: {0}")] Io(#[from] std::io::Error),
    #[error("invalid YAML: {0}")] Yaml(#[from] serde_yaml::Error),
    #[error("invalid config: {0}")] Validation(String),
}
...
impl Config {
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        if self.runtime.max_concurrent_tasks == 0 { return Err(ConfigError::Validation("runtime.max_concurrent_tasks must be > 0".into())); }
        if self.agents.is_empty() { return Err(ConfigError::Validation("at least one agent is required".into())); }
        let mut ids = std::collections::HashSet::new();
        for agent in &self.agents {
            if agent.id.trim().is_empty() || !ids.insert(&agent.id) { return Err(ConfigError::Validation(format!("invalid or duplicate agent id: {}", agent.id))); }
            if agent.limits.max_concurrent_tasks == 0 { return Err(ConfigError::Validation(format!("agent {} max_concurrent_tasks must be > 0", agent.id))); }
            if let AgentTransportConfig::HttpSse { endpoint, allow_http_development, .. } = &agent.transport {
                if !*allow_http_development && !endpoint.starts_with("https://") { return Err(ConfigError::Validation(format!("agent {} HTTP/SSE endpoint must use https", agent.id))); }
            }
        }
        if let StorageConfig::Postgres { schema, .. } = &self.storage {
            if schema.is_empty() || !schema.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') { return Err(ConfigError::Validation("postgres schema must match [A-Za-z0-9_]+".into())); }
        }
        Ok(())
    }
...
fn default_sqlite_path() -> PathBuf { PathBuf::from("./data/adapter.db") }
fn default_schema() -> String { "agent_adapter".into() }
fn default_pg_connections() -> u32 { 10 }
fn default_global_tasks() -> usize { 32 }
fn default_cleanup_seconds() -> u64 { 3_600 }
fn default_shutdown_seconds() -> u64 { 30 }
fn default_task_days() -> u64 { 7 }
fn default_event_days() -> u64 { 7 }
fn default_key_hours() -> u64 { 24 }
fn default_cleanup_batch() -> u32 { 1_000 }
fn default_agent_tasks() -> usize { 4 }
fn default_agent_queue() -> usize { 32 }
fn default_input_bytes() -> usize { 1024 * 1024 }
fn default_event_bytes() -> usize { 256 * 1024 }
fn default_task_timeout() -> u64 { 900 }
