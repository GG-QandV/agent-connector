...
struct ActiveAcpTask {
    ...
    #[allow(dead_code)] // зарезервировано для будущего использования session_id в provide_input/cancel-разграничении по сессии
...
fn spawn_reader_loop(
    ...
    tokio::spawn(async move {
            ...
            match lines.next_line().await {
                Ok(Some(line)) => {
                    ...
                    if let Some(id) = value.get("id").and_then(|v| v.as_u64()) {
                        ...
                        tracing::debug!(method, "received unhandled ACP push notification (v1 uses polling instead)");
                    }
                }
                Ok(None) => {
        ...
        for entry in active_tasks.iter() {
            let _ = entry.value().event_tx.send(DriverEvent::Failed(PublicError {
...
fn acp_block_to_part(block: &Value) -> Option<Part> {
    ...
    match kind {
        ...
        "resource" => {
            ...
            let mime_type = block.get("mimeType").and_then(|v| v.as_str()).map(str::to_string);
...
impl AgentDriver for AcpClientDriver {
    ...
    async fn invoke(
        ...
        let session_response = self.call("session/new", json!({}))
            .await
            .map_err(|e| CoreError::Driver(format!("session/new failed: {e}")))?;
        let session_id = session_response.get("sessionId")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());

        let (tx, rx) = mpsc::channel(32);
        self.active_tasks.insert(task_id, ActiveAcpTask {
            event_tx: tx.clone(),
        ...
        tokio::spawn(async move {
            match call_result {
                Ok(_) => { let _ = tx.send(DriverEvent::Accepted).await; }
                Err(e) => {
                    let _ = tx.send(DriverEvent::Failed(PublicError {
                        code: "acp_prompt_failed".into(), message: e.to_string(), retryable: true,
                    })).await;
            ...
            loop {
                ...
                let Ok(response) = tokio::time::timeout(std::time::Duration::from_secs(5), rrx).await else { continue };
                let Ok(response) = response else { break };
                let Some(events) = response.get("events").and_then(|v| v.as_array()) else { continue };
                ...
                for event in events {
                    ...
                    let driver_event = match kind {
                        "Progress" => Some(DriverEvent::Progress {
                            ...
                            percent: event.get("percent").and_then(|v| v.as_u64()).map(|p| p as u8),
                        }),
                        "InputRequired" => Some(DriverEvent::InputRequired(InputRequest {
                            question: event.get("question").and_then(|v| v.as_str()).unwrap_or("input required").to_string(),
                        })),
                        ...
                        "Failed" => Some(DriverEvent::Failed(PublicError {
                            ...
                            message: event.get("error").and_then(|v| v.get("message")).and_then(|v| v.as_str()).unwrap_or("remote ACP task failed").to_string(),
                            retryable: false,
                        })),
                    ...
                    if let Some(de) = driver_event { let _ = tx.send(de).await; }
                    if is_terminal { poll_active_tasks.remove(&poll_task_id); return; }
                }
            }
...
mod tests {
    ...
    fn extract_output_parts_uses_filter_map_without_type_mismatch() {
        ...
        assert_eq!(parts.len(), 2, "unknown kind must be filtered out, not error");
    }
}
