...
impl TaskStore for MemoryTaskStore {
    async fn create_or_get_idempotent(&self, task: NewTask) -> Result<CreateTaskResult, StoreError> {
        ...
        if let Some(task_id) = state.idempotency.get(&key) {
            let existing = state.tasks.get(task_id)
                .cloned().ok_or_else(|| StoreError::Corrupt("idempotency references absent task".into()))?;
    ...
    async fn append_event_and_transition(&self, transition: TaskTransition) -> Result<AppliedTransition, StoreError> {
        let mut state = self.state.lock().await;
        let snapshot = state.tasks.get_mut(&transition.task_id).ok_or(StoreError::NotFound(transition.task_id))?;
        if snapshot.revision != transition.expected_revision { return Err(StoreError::Conflict); }
        if !transition.allowed_states.contains(&snapshot.state) {
            return Err(StoreError::InvalidTransition(format!("current={:?}", snapshot.state)));
        }
        let now = Utc::now();
        ...
        if snapshot.state.terminal() { snapshot.terminal_at = Some(now); }
        let event = CoreEvent { task_id: transition.task_id, seq: snapshot.last_seq, at: now, kind: transition.event_kind };
        state.events.entry(transition.task_id).or_default().push(event.clone());
        Ok(AppliedTransition { snapshot: snapshot.clone(), event })
    }

    async fn events_after(&self, task_id: TaskId, after_seq: EventSeq, limit: u32) -> Result<Vec<CoreEvent>, StoreError> {
        let state = self.state.lock().await;
        if !state.tasks.contains_key(&task_id) { return Err(StoreError::NotFound(task_id)); }
        Ok(state.events.get(&task_id).into_iter().flatten()
            .filter(|event| event.seq > after_seq).take(limit as usize).cloned().collect())
    }

    async fn acquire_lease(&self, lease: Lease) -> Result<bool, StoreError> {
        let mut state = self.state.lock().await;
        let now = Utc::now();
        let expires_at = now + chrono::Duration::from_std(lease.expires_in).map_err(|error| StoreError::Internal(error.to_string()))?;
        match state.leases.get(&lease.task_id) {
            Some(existing) if existing.expires_at > now && existing.owner_id != lease.owner_id => Ok(false),
            _ => {
                state.leases.insert(lease.task_id, LeaseRecord { owner_id: lease.owner_id, expires_at });
                Ok(true)
            }
        }
    }
    ...
    async fn cleanup(&self, policy: &RetentionPolicy) -> Result<CleanupReport, StoreError> {
        let mut state = self.state.lock().await;
        let now = Utc::now();
        let task_cutoff = now - chrono::Duration::from_std(policy.task_ttl).map_err(|error| StoreError::Internal(error.to_string()))?;
        let event_cutoff = now - chrono::Duration::from_std(policy.event_ttl).map_err(|error| StoreError::Internal(error.to_string()))?;
        let expired_tasks: Vec<TaskId> = state.tasks.iter()
            .filter_map(|(id, snapshot)| snapshot.terminal_at.filter(|at| *at < task_cutoff).map(|_| *id))
            .take(policy.cleanup_batch_size as usize).collect();
        let mut report = CleanupReport::default();
        for task_id in expired_tasks {
            if let Some(snapshot) = state.tasks.remove(&task_id) {
                state.idempotency.remove(&(snapshot.caller_id, format!("memory:{}", task_id)));
