...
use adapterd_config::{AgentTransportConfig, Config, StorageConfig};
...
#[derive(Error, Debug)]
enum StartupError {
    #[error("config error: {0}")] Config(#[from] adapterd_config::ConfigError),
    #[error("storage error: {0}")] Storage(String),
    #[error("driver error: {0}")] Driver(String),
    #[error("environment variable is missing: {0}")] MissingEnv(String),
}
        ...
        let cleanup_task = tokio::spawn(async move {
            ...
            loop {
                ...
                match cleanup_store.cleanup(&retention).await {
                    Ok(report) => tracing::info!(tasks=report.tasks_deleted, events=report.events_deleted, "retention cleanup complete"),
                    Err(error) => tracing::warn!(%error, "retention cleanup failed"),
                }
            }
        });
        Ok(Self { config, core, cleanup_task })
    }
...
async fn build_store(config: &Config) -> Result<Arc<dyn TaskStore>, StartupError> {
    match &config.storage {
        ...
        StorageConfig::Sqlite { path } => {
            ...
            let store = SqliteTaskStore::open(path).await.map_err(|e| StartupError::Storage(e.to_string()))?;
            Ok(Arc::new(store))
        }
        StorageConfig::Postgres { dsn_env, schema, max_connections } => {
            ...
            let store = PostgresTaskStore::connect(&dsn, schema, *max_connections).await
...
async fn build_driver(agent: &adapterd_config::AgentConfig) -> Result<Arc<dyn AgentDriver>, StartupError> {
    match &agent.transport {
        AgentTransportConfig::Stdio { command, args, working_dir, env } => {
            let config = StdioDriverConfig {
                id: agent.id.clone(), command: command.clone(), args: args.clone(),
                working_dir: working_dir.clone(), env: env.clone(), ..Default::default()
            };
            Ok(Arc::new(StdioDriver::new(config)))
        }
        AgentTransportConfig::HttpSse { endpoint, token_env, allow_http_development } => {
            let mut config = HttpSseDriverConfig::new(Url::parse(endpoint)
                .map_err(|e| StartupError::Driver(format!("invalid endpoint for {}: {e}", agent.id)))?);
            config.id = agent.id.clone();
            config.require_https = !allow_http_development;
            config.credential = match token_env {
                Some(name) => Credential::Bearer(env::var(name).map_err(|_| StartupError::MissingEnv(name.clone()))?),
                None => Credential::None,
            };
            let driver = HttpSseDriver::new(config).map_err(|e| StartupError::Driver(e.to_string()))?;
            Ok(Arc::new(driver))
        }
    }
}

fn ensure_parent_dir(path: &PathBuf) -> Result<(), StartupError> {
