...
impl StdioDriver {
    ...
    async fn start_process(&self) -> Result<ProcessState, CoreError> {
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| CoreError::Driver("agent stdin unavailable".into()))?;
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| CoreError::Driver("agent stdout unavailable".into()))?;
    ...
    fn spawn_stdout_reader(
        ...
        tokio::spawn(async move {
            ...
            for (_, pending_task) in tasks.drain() {
                let _ = pending_task
                    .events
                    .try_send(DriverEvent::Failed(PublicError {
    ...
    async fn process(
        &self,
    ) -> Result<tokio::sync::MutexGuard<'_, Option<ProcessState>>, CoreError> {
...
impl AgentDriver for StdioDriver {
    ...
    async fn health(&self) -> Result<(), CoreError> {
        ...
        let state = guard
            .as_ref()
            .ok_or_else(|| CoreError::Driver("agent process unavailable".into()))?;
    ...
    async fn invoke(
        ...
        state
            ...
            .lock()
            .await
            .insert(task_id, PendingTask { events: tx });
        ...
        if let Err(error) =
            write_command(state.stdin.clone(), command, self.config.max_line_bytes).await
        {
...
async fn write_command<T: Serialize>(
    ...
    if encoded.len() > max_line_bytes {
        return Err(CoreError::InvalidRequest(
            "UAIC command exceeds max_line_bytes".into(),
        ));
    }
    encoded.push(b'\n');
