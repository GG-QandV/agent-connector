...
impl TaskStore for SqliteTaskStore {
    ...
    async fn create_or_get_idempotent(
        ...
        if let Some(row) =
            sqlx::query("SELECT * FROM tasks WHERE caller_id=? AND idempotency_key=?")
    ...
    async fn get_snapshot(&self, task_id: TaskId) -> Result<Option<TaskSnapshot>, StoreError> {
        sqlx::query("SELECT * FROM tasks WHERE task_id=?")
    ...
    async fn append_event_and_transition(
        ...
        let kind_json = serde_json::to_string(&transition.event_kind)
            .map_err(|e| StoreError::Internal(e.to_string()))?;
        sqlx::query(
            "INSERT INTO task_events(task_id,seq,event_kind_json,created_at) VALUES(?,?,?,?)",
        )
        .bind(transition.task_id.to_string())
        .bind(seq as i64)
        .bind(kind_json)
    ...
    async fn events_after(
        ...
        rows.into_iter()
            .map(|row| {
                let kind: CoreEventKind = serde_json::from_str(
                    &row.try_get::<String, _>("event_kind_json")
                        .map_err(db_error)?,
                )
                ...
                Ok(CoreEvent {
                    ...
                    at: parse_time(&row.try_get::<String, _>("created_at").map_err(db_error)?)?,
                    kind,
                })
            })
            .collect()
...
fn parse_time(value: &str) -> Result<DateTime<Utc>, StoreError> {
    value
        .parse()
        .map_err(|e| StoreError::Corrupt(format!("time: {e}")))
}
fn state_db(state: TaskState) -> &'static str {
    match state {
...
fn state(value: &str) -> Result<TaskState, StoreError> {
    match value {
        ...
        _ => Err(StoreError::Corrupt(format!("state={value}"))),
    }
}
fn snapshot(row: &sqlx::sqlite::SqliteRow) -> Result<TaskSnapshot, StoreError> {
    let id: String = row.try_get("task_id").map_err(db_error)?;
    let session: Option<String> = row.try_get("session_id").map_err(db_error)?;
    let terminal: Option<String> = row.try_get("terminal_at").map_err(db_error)?;
    Ok(TaskSnapshot {
        task_id: id
            .parse()
        ...
        session_id: session
            .map(|v| {
                v.parse()
                    .map_err(|e| StoreError::Corrupt(format!("session:{e}")))
            })
            .transpose()?,
        ...
        updated_at: parse_time(&row.try_get::<String, _>("updated_at").map_err(db_error)?)?,
        terminal_at: terminal.map(|v| parse_time(&v)).transpose()?,
    })
}
