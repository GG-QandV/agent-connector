...
use adapter_core::{
    AgentDriver, ArtifactRef, CoreError, DriverCapabilities, DriverEvent, InputRequest,
    InvokeRequest, Part, PublicError, TaskId,
};
...
impl HttpSseDriver {
    pub fn new(config: HttpSseDriverConfig) -> Result<Self, CoreError> {
        if config.require_https && config.base_url.scheme() != "https" {
            return Err(CoreError::InvalidRequest(
                "remote HTTP/SSE driver requires https".into(),
            ));
        }
    ...
    async fn stream_events(&self, task_id: TaskId, tx: mpsc::Sender<DriverEvent>) {
    ...
    async fn read_sse(
        ...
        loop {
            let next = if first {
                first = false;
                time::timeout(self.config.first_event_timeout, bytes.next())
                    .await
                    .map_err(|_| CoreError::Driver("first SSE event timeout".into()))?
            } else {
                time::timeout(self.config.idle_stream_timeout, bytes.next())
                    .await
                    .map_err(|_| CoreError::Driver("idle SSE stream timeout".into()))?
            };
            let Some(chunk) = next else {
                return Ok(StreamOutcome::Disconnected(latest_seq));
            };
            let chunk =
                chunk.map_err(|error| CoreError::Driver(format!("SSE read failure: {error}")))?;
            buffer.extend_from_slice(&chunk);
            if buffer.len() > self.config.max_event_bytes {
                return Err(CoreError::Driver(
                    "SSE event exceeds max_event_bytes".into(),
                ));
            }
            while let Some(boundary) = find_sse_boundary(&buffer) {
                let frame = buffer.drain(..boundary).collect::<Vec<_>>();
                drain_boundary(&mut buffer);
                let Some((id, data)) = parse_sse_frame(&frame) else {
                    continue;
                };
                let event: UaicEvent = serde_json::from_slice(&data).map_err(|error| {
                    CoreError::Driver(format!("invalid UAIC SSE event: {error}"))
                })?;
...
impl AgentDriver for HttpSseDriver {
    ...
    async fn invoke(
        ...
        tokio::spawn(async move {
            driver.stream_events(task_id, tx).await;
        });
        Ok(rx)
    }
    ...
    async fn cancel(&self, task_id: TaskId) -> Result<(), CoreError> {
        ...
        if !manifest.capabilities.cancellation {
            return Err(CoreError::InvalidRequest(
                "agent does not support cancellation".into(),
            ));
        }
    ...
    async fn provide_input(&self, task_id: TaskId, input: Vec<Part>) -> Result<(), CoreError> {
        ...
        if !manifest.capabilities.provide_input {
            return Err(CoreError::InvalidRequest(
                "agent does not support provide_input".into(),
            ));
        }
...
async fn map_uaic_event(
    ...
    let mapped = match event.message_type.as_str() {
        ...
        "artifact" => DriverEvent::Artifact(
            event
                .artifact
                .ok_or_else(|| CoreError::Driver("artifact event without artifact".into()))?,
        ),
        "input_required" => DriverEvent::InputRequired(
            event
                .request
                .ok_or_else(|| CoreError::Driver("input_required without request".into()))?,
        ),
    ...
    tx.send(mapped)
        .await
        .map_err(|_| CoreError::Driver("task consumer closed".into()))?;
    Ok(terminal)
}
