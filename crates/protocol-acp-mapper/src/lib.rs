...
impl<C: AcpCoreService> AcpMapper<C> {
    ...
    pub fn initialize_result(&self) -> &AcpInitializeResult {
        &self.initialize
    }
    ...
    pub async fn prompt(
        &self,
        caller: Caller,
        prompt: AcpSessionPrompt,
    ) -> Result<AcpTaskRef, AcpMapperError> {
        if prompt.request_id.trim().is_empty() {
            return Err(AcpMapperError::InvalidRequest(
                "request_id is required for idempotency".into(),
            ));
        }
        ...
        let snapshot = match result {
            ...
            _ => {
                return Err(AcpMapperError::InvalidRequest(
                    "unexpected core result for prompt".into(),
                ))
            }
        };
        Ok(AcpTaskRef {
    ...
    pub async fn cancel(&self, caller: Caller, task_id: &str) -> Result<(), AcpMapperError> {
        ...
        self.core
            .dispatch(
                ...
                CoreCommand::Cancel {
                    task_id: id,
                    reason: Some("ACP client cancellation".into()),
                },
            )
            .await?;
        Ok(())
    ...
    pub async fn provide_input(
        ...
        let input = content
            ...
            .collect::<Result<_, _>>()?;
        self.core
            .dispatch(caller, CoreCommand::ProvideInput { task_id: id, input })
            .await?;
        Ok(())
    }
...
fn map_event(event: CoreEvent) -> Vec<AcpSessionUpdate> {
    ...
    let update = match event.kind {
        CoreEventKind::Accepted { .. } => AcpUpdate::Progress {
        ...
        CoreEventKind::Completed { output } => {
            return vec![
                AcpSessionUpdate {
                    ...
                    update: AcpUpdate::AgentMessage {
                        content: output.into_iter().map(from_core_part).collect(),
                    },
                },
                AcpSessionUpdate {
                    session_id,
                    task_id,
                    update: AcpUpdate::Completed,
                },
            ]
        }
        CoreEventKind::Failed { error } => AcpUpdate::Failed {
            code: error.code,
            message: error.message,
        },
        CoreEventKind::CancelRequested { .. } => AcpUpdate::Progress {
...
fn parse_uuid(value: &str) -> Result<Uuid, AcpMapperError> {
    value
        .parse()
        .map_err(|_| AcpMapperError::InvalidRequest("expected UUID".into()))
}
